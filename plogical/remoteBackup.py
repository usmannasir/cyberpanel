...
class remoteBackup:
    ...
    def startRestore(backupDir,backupLogPath,dir):
        try:
            ...
            for backup in os.listdir(backupDir):
                ...
                if backup.endswith(ext):
                    ...
                    writeToFile.writelines("[" + time.strftime(
                        "%H-%M-%S-%b-%d-%Y") + "]" + " Starting restore for: "+backup+".\n")
                    writeToFile.close()
                    ...
                    if data['restoreStatus'] == 1:
                        while (1):
                            ...
                            if data['abort'] == 1 and data['running'] == "Error":
                                ...
                                writeToFile.writelines("\n")
                                writeToFile.writelines("[" + time.strftime(
                                    "%H-%M-%S-%b-%d-%Y") + "]" + " Restore aborted for: " + backup + ". Error message: "+data['status']+"\n")
                                writeToFile.writelines("[" + time.strftime(
                                    "%H-%M-%S-%b-%d-%Y") + "]" + " #########################################\n")
                                writeToFile.close()
                                break
                                ...
                                writeToFile.writelines("\n")
                                writeToFile.writelines("[" + time.strftime(
                                    "%H-%M-%S-%b-%d-%Y") + "]" + " Restore Completed for: " + backup + ".\n")
                                writeToFile.writelines("[" + time.strftime(
                                    "%H-%M-%S-%b-%d-%Y") + "]" + " #########################################\n")
                                writeToFile.close()
                                break
                                ...
                                writeToFile.writelines("\n")
                                writeToFile.writelines("[" + time.strftime(
                                    "%H-%M-%S-%b-%d-%Y") + "]" + " Waiting for restore to complete.\n")
                                writeToFile.close()
                                time.sleep(3)
                                pass
                    else:
                        ...
                        writeToFile.writelines("\n")
                        writeToFile.writelines("[" + time.strftime(
                            "%H-%M-%S-%b-%d-%Y") + "]" + "Could not start restore process for: " + backup + "\n")
                        writeToFile.close()
            ...
            writeToFile.writelines("\n")
            writeToFile.writelines("[" + time.strftime(
                "%H-%M-%S-%b-%d-%Y") + "]" + " Backup Restore complete\n")
    ...
    def createBackup(virtualHost, ipAddress,writeToFile, dir):
        try:
            ...
            writeToFile.writelines("["+time.strftime("%H-%M-%S-%b-%d-%Y")+"]"+" Preparing to create backup for: "+virtualHost+"\n")
            writeToFile.writelines("[" + time.strftime(
                "%H-%M-%S-%b-%d-%Y") + "]" + " Backup started for: " + virtualHost + "\n")
            ...
            writeToFile.writelines("[" + time.strftime(
                "%H-%M-%S-%b-%d-%Y") + "]" + " Backup created for:" + virtualHost + "\n")

            writeToFile.writelines("[" + time.strftime(
                "%H-%M-%S-%b-%d-%Y") + "]" + " Preparing to send backup for: " + virtualHost +" to "+ipAddress+ "\n")
            writeToFile.flush()

            remoteBackup.sendBackup(backupPath+".tar.gz", ipAddress,writeToFile, dir)

            writeToFile.writelines("[" + time.strftime(
                "%H-%M-%S-%b-%d-%Y") + "]" + "  Backup for: " + virtualHost + " is sent to " + ipAddress + "\n")
        ...
        try:
            ...
            for virtualHost in accountsToTransfer:
                ...
                try:
                    ...
                    writeToFile = open(backupLogPath, "a")
                    writeToFile.writelines("[" + time.strftime(
                        "%H-%M-%S-%b-%d-%Y") + "]" + " Currently generating local backups for: " + virtualHost + "\n")
                    writeToFile.close()
                    ...
                    while (1):
                        ...
                        writeToFile.writelines("[" + time.strftime(
                            "%H-%M-%S-%b-%d-%Y") + "]" + " Waiting for backup to complete.. " + "\n")
                        ...
                        if data['abort'] == 1:
                            ...
                            writeToFile.writelines("[" + time.strftime(
                                "%H-%M-%S-%b-%d-%Y") + "]" + " Local Backup Completed for: " +virtualHost + " with status: "+ data['status'] +"\n")
                            ...
                            writeToFile.writelines("[" + time.strftime(
                                "%H-%M-%S-%b-%d-%Y") + "]" + " Sending " + completedPathToSend +" to "+ipAddress +".\n")


                            remoteBackup.sendBackup(completedPathToSend,ipAddress,str(folderNumber),writeToFile)

                            writeToFile.writelines("[" + time.strftime(
                                "%H-%M-%S-%b-%d-%Y") + "]" + " Sent " + completedPathToSend + " to " + ipAddress + ".\n")
            ...
            writeToFile = open(backupLogPath, "a")
            writeToFile.writelines("[" + time.strftime(
                "%H-%M-%S-%b-%d-%Y") + "]" + " Backups are successfully generated and received on: " + ipAddress + "\n")
            writeToFile.close()
    ...
    def remoteTransfer(ipAddress, dir,accountsToTransfer):
        try:
            ...
            if backupUtil.backupUtilities.checkIfHostIsUp(ipAddress) == 1:
                ...
                if checkConn[0] == 0:
                    writeToFile.writelines("[" + time.strftime(
                        "%H-%M-%S-%b-%d-%Y") + "]" + " Connection to:" + ipAddress + " Failed, please resetup this destination from CyberPanel, aborting." + "\n")
                    writeToFile.close()
                ...
                writeToFile.writelines("[" + time.strftime(
                    "%H-%M-%S-%b-%d-%Y") + "]" + " Host:" + ipAddress + " is down, aborting." + "\n")
                writeToFile.close()
