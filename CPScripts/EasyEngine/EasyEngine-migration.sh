...
export_database() {
WPDBNAME=$(grep DB_NAME /opt/easyengine/sites/"${domains[$i]}"/app/wp-config.php | cut -d \' -f 4)
WPDBUSER=$(grep DB_USER /opt/easyengine/sites/"${domains[$i]}"/app/wp-config.php | cut -d \' -f 4)
WPDBPASS=$(grep DB_PASSWORD /opt/easyengine/sites/"${domains[$i]}"/app/wp-config.php | cut -d \' -f 4)
