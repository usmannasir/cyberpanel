#!/usr/bin/env bash
# Run inside the guest after provision, or via: vagrant ssh cp-fresh -c 'sudo bash /vagrant/smoke.sh'
set -euo pipefail
fail=0
...
# Source-level check matches Test/test_get_php_string.py (no Django required).
